use crate::db;
use crate::dto::{
    AddGroupMemberRequest, CreateGroupRequest, UpdateGroupRequest, UpdateMemberAliasRequest,
    UpdateMemberRoleRequest,
};

use crate::models::{ChatMessage, ImGroup, ImGroupMember, ImGroupMessage, User};
...
pub async fn add_group_member(
    ...
    if let Ok(from_user) = depot.obtain::<User>() {
        ...
        // 在添加成员前，检查当前成员数
        let current_members: Vec<ImGroupMember> = im_group_service::get_group_members(&group_id)
            .await
            .unwrap_or_default();
            ...
            match im_group_service::get_group(&group_id).await {
                ...
                Err(_) => {
                    ...
                    let new_group = ImGroup {
                        ...
                        group_type: 1,                  // 私有群
                        group_name: "群聊".to_string(), // 默认名称，前端可以修改
                        mute: Some(0),
                        apply_join_type: 1,
                        avatar: None,
...
pub async fn remove_group_member(
    ...
    if let Ok(from_user) = depot.obtain::<User>() {
        ...
        im_group_service::remove_group_member(&group_id, &member_id, &operator_id).await?;
...
pub async fn delete_group(
    ...
    if let Ok(from_user) = depot.obtain::<User>() {
        ...
        im_group_service::delete_group(&group_id, &owner_id).await?;
...
pub async fn dissolve_group(
    ...
    if let Ok(from_user) = depot.obtain::<User>() {
        let publisher = crate::mqtt::get_mqtt_publisher();
        let group_id = group_id.into_inner();
        let owner_id = from_user.open_id.clone();
