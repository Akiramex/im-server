...
pub async fn update_group(
    ...
    if let Some(ref group_name) = req.group_name {
        if group_name.trim().is_empty() {
            warn!("群组名称不能为空");
            return Err(AppError::public("群组名称不能为空".to_string()));
        }
    ...
    if let Some(max_member_count) = req.max_member_count {
        if max_member_count < 1 {
            warn!("最大成员数必须大于0");
            return Err(AppError::public("最大成员数必须大于0".to_string()));
        }
        if need_comma {
            query_builder.push(", ");
        }
    ...
    if !has_update {
        warn!("没有需要更新的字段");
        return Err(AppError::public("没有需要更新的字段".to_string()));
    }
