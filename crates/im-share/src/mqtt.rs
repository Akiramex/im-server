                    ...
                    Ok(event) => {
                        match event {
                            Event::Incoming(Packet::Publish(p)) => {
                                ...
                                let message_info = if let Ok(text) =
                                ...
                                if receiver_count == 0 {
                                    ...
                                    // 有订阅者，尝试发送
                                    match tx_clone.send(IncomingMessage {
                                        topic: p.topic,
                            ...
                            Event::Incoming(Packet::SubAck(sa)) => {
                                info!(
                                    packet_id = sa.pkid,
                                    "✅ MQTT 订阅确认（broker将推送离线消息，QoS 1 + clean_session=false）"
                                );
                            ...
                            Event::Outgoing(_) => {
                                // 忽略出站事件
                                debug!("收到 MQTT Outgoing 包")
                            }
                            _ => {
                                // 其他事件可以在这里处理
                                // 打印出包的类型
                                debug!("收到 其他 MQTT 包 {:?}", event);
                            }
                        }
                    }
                    Err(e) => {
    ...
    pub async fn publish(&self, topic: &str, payload: Vec<u8>) -> Result<()> {
        ...
        let message_info = if let Ok(text) = String::from_utf8(payload.clone()) {
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(&text) {
                format!(
                    "message_id={:?}, from_user_id={:?}, to_user_id={:?}, chat_type={:?}",
                    json.get("message_id"),
                    json.get("from_user_id"),
                    json.get("to_user_id"),
                    json.get("chat_type")
